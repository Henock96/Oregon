...
const OvulationCalculator: React.FC = () => {
  ...
  return (
    <Box p={4}>
      <Input
        type="number"
        placeholder="Durée du Cycle Menstruel (jours)"
      ...
      Calculer la fenêtre d&apos;ovulation
      </Button>
      {ovulationWindowStart && ovulationWindowEnd && (
        <Text mt={4}>
          Fenêtre d&apos;ovulation: {format(ovulationWindowStart, 'MMMM d')} - {format(ovulationWindowEnd, 'MMMM d')}
        </Text>
      )}
    </Box>
  );
